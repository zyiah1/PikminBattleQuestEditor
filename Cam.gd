	...
	if event is InputEventMouseButton:
		match event.button_index:
			...
			MOUSE_BUTTON_WHEEL_UP: # Increases max velocity
				_vel_multiplier = clamp(_vel_multiplier * 1.1, 1, 80)
			MOUSE_BUTTON_WHEEL_DOWN: # Decereases max velocity
				_vel_multiplier = clamp(_vel_multiplier / 1.1, 1, 80)
	# Receives key input
	
	_w = Input.is_action_pressed("forward")
	_s = Input.is_action_pressed("backward")
...
func _update_movement(delta):
	if get_parent().get_node("UI/Properties").position.x > 1500:
		if not Input.is_action_pressed("Cmd"):
			if paused == false:
